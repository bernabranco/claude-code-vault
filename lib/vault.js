...
export class Vault {
  ...
  async _indexFile(filePath) {
    try {
      ...
      // Internal-only: lowercased body for substring matching in search() (#56).
      // Defined non-enumerable so `{ ...note }`, `JSON.stringify(note)`, and
      // `Object.keys(note)` all naturally skip it — external wire formats (MCP
      // payloads, exportAsJson, viewer API) cannot leak note bodies by accident,
      // even if a future method returns notes without an explicit field allowlist.
      // Roughly doubles in-memory index size; acceptable for vaults < ~10k notes.
      Object.defineProperty(entry, "_bodyLower", {
  ...
  getGraph() {
    ...
    const nodes = this.index.map((n) => ({ ...n }));
